...
sub Protein {
    ...
    foreach my $variant (keys %variants){
        ...
        my $response_parsed = RESTsubmit::REST($URL);

        # If there was some problem:
        next unless ref $response_parsed eq "ARRAY";
...
sub _UniProtData {
    ...
    my $URL = "http://www.uniprot.org/uniprot/?query=id:".$line."&columns=id";

    # Required fields:
    $URL .= "%2Ccomment%28FUNCTION%29"; # Function
    $URL .= "%2Ccomment%28SUBUNIT%29"; # Subunit
    $URL .= "%2Ccomment%28DEVELOPMENTAL%20STAGE%29"; # Developmental stage
    $URL .= "%2Ccomment%28TISSUE%20SPECIFICITY%29"; # Tissue specificity
    $URL .= "%2Ccomment%28CATALYTIC%20ACTIVITY%29"; # Catalytic activity
    $URL .= "%2Ccomment%28DISRUPTION%20PHENOTYPE%29"; # Disruption phenotype
    $URL .= "%2Ccomment%28SUBCELLULAR%20LOCATION%29"; # Cellular localization
    $URL .= "%2Ccomment%28DISEASE%29"; # Disease
    $URL .= "%2Centry%20name"; # Entry name
    $URL .= "&format=tab"; # tab delimited format
